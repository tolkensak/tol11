...
#ifndef _INC_TOLMYSQL_CONNECTION_H
#define _INC_TOLMYSQL_CONNECTION_H
...
class TOLMYSQLDECL Connection : public SmartObject
{
//private:
...
public:
	Connection();
	Connection(PCCharA pcHost, PCCharA pcUser, PCCharA pcPassword, PCCharA pcDB, Uint uPort=0, PCCharA pcUnixSocket=NULL, Ulong ulClientFlags=0);
	virtual ~Connection();
	...
	ResultPtr ListDB(PCCharA pcWild) _CONST_;
	ResultPtr ListFields(PCCharA pcTable, PCCharA pcWild) _CONST_;
	ResultPtr ListProcesses() _CONST_;
	ResultPtr ListTables(PCCharA pcWild) _CONST_;
	...
	int RealQuery(StringA strStmt) _CONST_;
	...
	ResultPtr StoreResult() _CONST_;
	Ulong ThreadId() _CONST_;
	ResultPtr UseResult() _CONST_;
...
#ifndef _TOL_MYSQLCONNECTIONPTR
...
#include <tolmysql_connection.inl>

#endif /* _INC_TOLMYSQL_CONNECTION_H */
