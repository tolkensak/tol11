...
BEGIN_MESSAGE_MAP(TCtrlBar, TCtrlBarGrip)
...
void TCtrlBar::OnUpdateCmdUI(CFrameWnd* pTarget, BOOL bDisableIfNoHndler)
{
	TCtrlBarGrip::OnUpdateCmdUI(pTarget, bDisableIfNoHndler);
...
void TCtrlBar::NcPaintGripper(CDC* pDC, CRect rcClient)
{
	if(!HasGripper())
		return;
	...
	// draw the button
	m_btnHide.Paint(pDC, m_bActive ?
			::GetSysColor(COLOR_CAPTIONTEXT) :
			::GetSysColor(COLOR_INACTIVECAPTIONTEXT));
}

LRESULT TCtrlBar::OnSetText(WPARAM wParam, LPARAM lParam)
{
	LRESULT lResult=TCtrlBarGrip::OnSetText(wParam, lParam);
	SendMessage(WM_NCPAINT);
	return lResult;
}
