...
// TCtrlBarSize::TCtrlBarKeepSize

TCtrlBarSize::TCtrlBarKeepSize::TCtrlBarKeepSize(TCtrlBarSize* pBar)
	: m_pBar(pBar)
{
	if(pBar)
	{
		m_pBar=pBar;
		m_bOldKeepSize=m_pBar->m_bKeepSize;
		m_pBar->m_bKeepSize=TRUE;
	}
}

TCtrlBarSize::TCtrlBarKeepSize::~TCtrlBarKeepSize()
{
	if(m_pBar)
		m_pBar->m_bKeepSize=m_bOldKeepSize;
}


// TCtrlBarSize

IMPLEMENT_DYNAMIC(TCtrlBarSize, CControlBar)

BEGIN_MESSAGE_MAP(TCtrlBarSize, CControlBar)
	ON_WM_CREATE()
	ON_WM_PAINT()
	ON_WM_NCPAINT()
	ON_WM_NCCALCSIZE()
...
BOOL TCtrlBarSize::Create(LPCTSTR lpszWindowName, CWnd* pParentWnd, CSize sizeDefault, BOOL bHasGripper, UINT nID, DWORD dwStyle)
{
...
BOOL TCtrlBarSize::Create(LPCTSTR lpszWindowName, CWnd* pParentWnd, UINT nID, DWORD dwStyle)
{
	// must have a parent
...
//?
///////////////////////////////////////////////////////////////////////////
//// TCtrlBarSize operations
//#if defined(_SCB_REPLACE_MINIFRAME) && !defined(_SCB_MINIFRAME_CAPTION)
//void TCtrlBarSize::EnableDocking(DWORD dwDockStyle)
//{
...
//		m_pDockContext=new CSizingDockContext(this);
//
...
/////////////////////////////////////////////////////////////////////////
// TCtrlBarSize message handlers

int TCtrlBarSize::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	if(CControlBar::OnCreate(lpCreateStruct)==-1)
		return -1;

	// query SPI_GETDRAGFULLWINDOWS system parameter OnSettingChange() will update m_bDragShowContent
	m_bDragShowContent=FALSE;
...
LRESULT TCtrlBarSize::OnSetText(WPARAM wParam, LPARAM lParam)
{
...
CSize TCtrlBarSize::CalcFixedLayout(BOOL bStretch, BOOL bHorz)
{
	...
	TCtrlBarDockBar* pDockBar=(TCtrlBarDockBar*) m_pDockBar;
...
CSize TCtrlBarSize::CalcDynamicLayout(int nLength, DWORD dwMode)
{
...
void TCtrlBarSize::OnWindowPosChanging(WINDOWPOS FAR* lpwndpos)
{
...
void TCtrlBarSize::OnLButtonDown(UINT nFlags, CPoint point)
{
	if(m_pDockBar !=NULL)
	{
		// start the drag
...
void TCtrlBarSize::OnLButtonDblClk(UINT nFlags, CPoint point)
{
	if(m_pDockBar !=NULL)
	{
		// toggle docking
...
void TCtrlBarSize::OnNcLButtonDown(UINT nHitTest, CPoint point) 
{
	UNUSED_ALWAYS(point);
...
void TCtrlBarSize::OnLButtonUp(UINT nFlags, CPoint point)
{
	if(m_bTracking)
		StopTracking();
...
void TCtrlBarSize::OnRButtonDown(UINT nFlags, CPoint point)
{
	if(m_bTracking)
		StopTracking();
...
void TCtrlBarSize::OnMouseMove(UINT nFlags, CPoint point)
{
	if(m_bTracking)
	{
...
void TCtrlBarSize::OnCaptureChanged(CWnd *pWnd)
{
...
void TCtrlBarSize::OnNcCalcSize(BOOL bCalcValidRects, NCCALCSIZE_PARAMS FAR* lpncsp)
{
...
void TCtrlBarSize::NcCalcClient(LPRECT pRc, UINT nDockBarID)
{
	CRect rc(pRc);
...
void TCtrlBarSize::OnNcPaint()
{
	...
	mdc.FillRect(rcDraw, CBrush::FromHandle((HBRUSH)(LONG_PTR)GetClassLong(m_hWnd, -10))); // -10=GCL_HBRBACKGROUND
...
void TCtrlBarSize::NcPaintGripper(CDC* pDC, CRect rcClient)
{
	UNUSED_ALWAYS(pDC);
	UNUSED_ALWAYS(rcClient);
}

void TCtrlBarSize::OnPaint()
{
...
UINT
#endif
	TCtrlBarSize::OnNcHitTest(CPoint point)
{
	CRect rcBar, rcEdge;
	GetWindowRect(rcBar);
...
void TCtrlBarSize::OnClose()
{
	// do nothing: protection against accidentally destruction by the
	//   child control (i.e. if user hits Esc in a child editctrl)
}

// TCtrlBarSize implementation helpers

void TCtrlBarSize::StartTracking(UINT nHitTest, CPoint point)
{
	SetCapture();
	...
	if(!IsSideTracking())
	{
		...
		CRect rcT;
		m_pDockSite->RepositionBars(0, 0xFFFF, AFX_IDW_PANE_FIRST, reposQuery, &rcT, NULL, TRUE);
...
void TCtrlBarSize::StopTracking()
{
...
void TCtrlBarSize::OnTrackUpdateSize(CPoint& point)
{
	ASSERT(!IsFloating());
	...
	else
	{
		...
		// nGrowing is growing
		nDelta=abs(nDelta);
		TCtrlBarSize* pBar=arrSCBars[nGrowingBar];
...
void TCtrlBarSize::OnTrackInvertTracker()
{
	ASSERT(m_bTracking);
...
BOOL TCtrlBarSize::GetEdgeRect(CRect rcWnd, UINT nHitTest,
									CRect& rcEdge)
{
	rcEdge=rcWnd;
	if(m_dwSCBStyle & SCBS_SHOWEDGES)
...
UINT TCtrlBarSize::GetEdgeHTCode(int nEdge)
{
...
void TCtrlBarSize::GetRowInfo(int& nFirst, int& nLast, int& nThis)
{
...
void TCtrlBarSize::GetRowSizingBars(TSizeBarArray& arrSCBars)
{
	int nThis; // dummy
	GetRowSizingBars(arrSCBars, nThis);
}

void TCtrlBarSize::GetRowSizingBars(TSizeBarArray& arrSCBars, int& nThis)
{
	arrSCBars.RemoveAll();
	...
	{
		TCtrlBarSize* pBar=
			(TCtrlBarSize*) m_pDockBar->m_arrBars[i];
		if(HIWORD(pBar)==0) continue; // placeholder
		if(!pBar->IsVisible()) continue;
		if(pBar->IsKindOf(RUNTIME_CLASS(TCtrlBarSize)))
		{
			if(pBar==this)
...
BOOL TCtrlBarSize::NegotiateSpace(int nLengthTotal, BOOL bHorz)
{
	...
	{
		pBar=(TCtrlBarSize*) m_pDockBar->m_arrBars[i];
		if(HIWORD(pBar)==0) continue; // placeholder
		if(!pBar->IsVisible()) continue;
		BOOL bIsSizingBar=
			pBar->IsKindOf(RUNTIME_CLASS(TCtrlBarSize));
...
void TCtrlBarSize::AlignControlBars()
{
	int nFirst, nLast, nThis;
	GetRowInfo(nFirst, nLast, nThis);
	...
	{
		TCtrlBarSize* pBar=(TCtrlBarSize*) m_pDockBar->m_arrBars[i];
		if(HIWORD(pBar)==0)
...
void TCtrlBarSize::OnUpdateCmdUI(CFrameWnd* pTarget, BOOL bDisableIfNoHndler)
{
	UNUSED_ALWAYS(bDisableIfNoHndler);
	UNUSED_ALWAYS(pTarget);
}

void TCtrlBarSize::GetInfo(TCtrlBarInfo* pInfo)
{
	if(!pInfo)
		return;

	pInfo->sizeFloat=m_szFloat;
	pInfo->sizeHorz=m_szHorz;
	pInfo->sizeVert=m_szVert;
	pInfo->sizeMinFloat=m_szMinFloat;
	pInfo->sizeMinHorz=m_szMinHorz;
	pInfo->sizeMinVert=m_szMinVert;
}

void TCtrlBarSize::SetInfo(TCtrlBarInfo* pInfo, CFrameWnd* pFrameWnd)
{
	if(!pInfo || !pFrameWnd)
		return;

	m_szFloat=pInfo->sizeFloat;
	m_szHorz=pInfo->sizeHorz;
	m_szVert=pInfo->sizeVert;
	m_szMinFloat=pInfo->sizeMinFloat;
	m_szMinHorz=pInfo->sizeMinHorz;
	m_szMinVert=pInfo->sizeMinVert;
}

void TCtrlBarSize::LoadState(LPCTSTR lpszProfileName)
{
	ASSERT_VALID(this);
...
//?
	...
	TCHAR szSection[256];
	wsprintf(szSection, _T("%sSizeBar%d"), lpszProfileName, GetDlgCtrlID());
...
void TCtrlBarSize::SaveState(LPCTSTR lpszProfileName)
{
...
void TCtrlBarSize::GlobalLoadState(CFrameWnd* pFrame, LPCTSTR lpszProfileName)
{
	POSITION pos=pFrame->m_listControlBars.GetHeadPosition();
	while (pos !=NULL)
	{
		TCtrlBarSize* pBar=(TCtrlBarSize*) pFrame->m_listControlBars.GetNext(pos);
		ASSERT(pBar !=NULL);
		if(pBar->IsKindOf(RUNTIME_CLASS(TCtrlBarSize)))
...
void TCtrlBarSize::GlobalSaveState(CFrameWnd* pFrame, LPCTSTR lpszProfileName)
{
	POSITION pos=pFrame->m_listControlBars.GetHeadPosition();
	while (pos !=NULL)
	{
...
///////////////////////////////////////////////////////////////////////////////
//// CSizingDockContext Drag Operations
//
...
//}
//
//void CSizingDockContext::StartDrag(CPoint pt)
//{
//	ASSERT_VALID(m_pBar);
//	m_bDragging=TRUE;
//
...
//// CSizingDockFrameWnd
//
//IMPLEMENT_DYNCREATE(CSizingDockFrameWnd, CMiniDockFrameWnd);
//
//BEGIN_MESSAGE_MAP(CSizingDockFrameWnd, CMiniDockFrameWnd)
//	//{{AFX_MSG_MAP(CSizingDockFrameWnd)
//	ON_WM_NCLBUTTONDOWN()
//	ON_WM_GETMINMAXINFO()
//	ON_WM_WINDOWPOSCHANGING()
//	ON_WM_SIZE()
...
//END_MESSAGE_MAP()
//
//BOOL CSizingDockFrameWnd::Create(CWnd* pParent, DWORD dwBarStyle)
//{
...
//		NULL, _T("&afxChNil"), dwStyle, rectDefault, pParent))
//	{
...
//}
//
//void CSizingDockFrameWnd::OnNcLButtonDown(UINT nHitTest, CPoint point)
//{
...
//}
//
//TCtrlBarSize* CSizingDockFrameWnd::GetSizingControlBar()
//{
...
//		return NULL;
//
//	if(!pWnd->IsKindOf(RUNTIME_CLASS(TCtrlBarSize)))
//		return NULL;
//
//	return (TCtrlBarSize*) pWnd;
//}
//
//void CSizingDockFrameWnd::OnSize(UINT nType, int cx, int cy) 
//{
//	TCtrlBarSize* pBar=GetSizingControlBar();
...
//}
//
//void CSizingDockFrameWnd::OnGetMinMaxInfo(MINMAXINFO FAR* lpMMI)
//{
...
//}
//
//void CSizingDockFrameWnd::OnWindowPosChanging(WINDOWPOS FAR* lpwndpos) 
//{
